...
module.exports = function(client) {
		...
		if (resp.success) {
			...
			if (ip.length) {
				render(res, req, 'message.ejs', {
					message: `認証失敗しました。異なったアカウントで認証したため ${
			...
			render(res, req, 'message.ejs', {
				message: '認証失敗しました。サーバーに再度参加して認証し直してください。'
			});
			member.kick();
		}
	});
	...
	app.post('/api/prefix', (req, res) => {
		...
		if (!prefix)
			return res.json({
				success: false,
				alert: { title: 'ありぇ？', message: 'プレフィックスがないよ！', type: 'error' }
			});
		if (prefix.length > 4)
			return res.json({
				success: false,
	...
	app.post('/api/save', async (req, res) => {
		...
		try {
			...
			return res.json({
				...
				alert: {
					title: 'ありぇ？',
					message: '予期せぬエラーが発生しました。もう一度やり直してください。',
					type: 'error'
				}
			});
		}
